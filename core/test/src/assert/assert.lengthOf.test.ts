...
describe("assert.lengthOf", () => {
    describe("basic functionality", () => {
        it("should pass for arrays with correct length", () => {
            assert.lengthOf([1, 2, 3], 3);
            assert.lengthOf([], 0);
            assert.lengthOf([1], 1);
            assert.lengthOf([1, 2, 3, 4, 5], 5);
        });

        it("should pass for strings with correct length", () => {
        ...
        it("should fail for arrays with incorrect length", () => {
            checkError(
                () => assert.lengthOf([1, 2, 3], 2),
                "expected [1,2,3] to have a length of 2 but got 3"
            );
        });

        it("should fail for strings with incorrect length", () => {
            checkError(
        ...
        it("should work with custom message", () => {
            checkError(
                () => assert.lengthOf([1, 2], 3, "Array length mismatch"),
    ...
    describe("Map support", () => {
        ...
        it("should work with custom message for Maps", () => {
            ...
            checkError(
                () => assert.lengthOf(map, 2, "Map size mismatch"),
                "Map size mismatch"
            );
        });
    });
    ...
    describe("Set support", () => {
        ...
        it("should work with custom message for Sets", () => {
            ...
            checkError(
                () => assert.lengthOf(set, 2, "Set size mismatch"),
                "Set size mismatch"
            );
        });
    });
    ...
    describe("edge cases", () => {
        it("should fail for objects without length or size property", () => {
            checkError(
                () => assert.lengthOf({} as any, 0),
                "expected {} to have property \"length\" or \"size\""
            );

            checkError(
                () => assert.lengthOf({ a: 1 } as any, 1),
                /expected .+ to have property "length" or "size"/
            );
        });

        it("should fail for primitives without length", () => {
            checkError(
                () => assert.lengthOf(123 as any, 3),
                "expected 123 to have property \"length\" or \"size\""
            );

            checkError(
                () => assert.lengthOf(true as any, 1),
        ...
        it("should fail for null/undefined", () => {
            checkError(
                () => assert.lengthOf(null as any, 0),
                "expected null to have property \"length\" or \"size\""
            );

            checkError(
                () => assert.lengthOf(undefined as any, 0),
        ...
        it("should work with typed arrays", () => {
            assert.lengthOf(new Uint8Array(5), 5);
            assert.lengthOf(new Int32Array([1, 2, 3]), 3);
            assert.lengthOf(new Float64Array(10), 10);
        });

        it("should work with array-like objects", () => {
            const arrayLike = { 0: "a", 1: "b", length: 2 };
            assert.lengthOf(arrayLike, 2);
        });
    });
});

describe("assert.notLengthOf", () => {
    describe("basic functionality", () => {
        it("should pass when length doesn't match", () => {
            assert.notLengthOf([1, 2, 3], 2);
            assert.notLengthOf("hello", 4);
            assert.notLengthOf(new Map([["a", 1]]), 2);
            assert.notLengthOf(new Set([1, 2]), 3);
        });

        it("should fail when length matches", () => {
            checkError(
                () => assert.notLengthOf([1, 2, 3], 3),
                "not expected [1,2,3] to have a length of 3 but got 3"
            );

            checkError(
                () => assert.notLengthOf("hello", 5),
        ...
        it("should work with custom message", () => {
            checkError(
                () => assert.notLengthOf([1, 2], 2, "Should not have this length"),
    ...
    describe("expect syntax with negation", () => {
        it("should work with not operator", () => {
            expect([1, 2, 3]).to.not.have.lengthOf(2);
            expect("hello").to.not.have.lengthOf(4);
            expect(new Map([["a", 1]])).to.not.have.lengthOf(2);
            expect(new Set([1, 2])).to.not.have.lengthOf(3);
        });

        it("should fail when length matches with not operator", () => {
            checkError(
                () => expect([1, 2]).to.not.have.lengthOf(2),
    ...
    describe("edge cases", () => {
        it("should handle zero length correctly", () => {
            assert.notLengthOf([1], 0);
            assert.notLengthOf("a", 0);
            assert.notLengthOf(new Map([["a", 1]]), 0);
            assert.notLengthOf(new Set([1]), 0);
        });

        it("should not fail for objects without length or size property", () => {
            assert.notLengthOf({} as any, 0);
            assert.notLengthOf({ a: 1 } as any, 1);
            assert.notLengthOf(123 as any, 3);
            assert.notLengthOf(true as any, 1);
            assert.notLengthOf(null as any, 0);
            assert.notLengthOf(undefined as any, 0);
        });
    });
});
